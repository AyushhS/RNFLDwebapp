...
def detectLines(img_name):
    model = tf.keras.models.load_model(root + 'retinet_9010.h5')
    imgpath = img_name
    N = 32
    ...
    def draw_circle(event = None, x = None, y = None, flags = None, param = None):
        global cx, cy, rx, ry, drawingnum
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if drawingnum == 2:
                cv2.circle(img1, (cx, cy), 1, (0, 255, 0), -1)
            elif drawingnum == 1:
                cv2.circle(img1, (rx, ry), 1, (0, 255, 0), -1)
                cv2.circle(img1, (cx, cy), int(3 * sqrt((cx - rx) * (cx - rx) + (cy - ry) * (cy - ry))), (0, 255, 0), 1)
            drawingnum = drawingnum - 1

    (h, w) = img1.shape
    cv2.namedWindow('Marking ROI')
    cv2.setMouseCallback('Marking ROI', draw_circle)

    while True:
        cv2.imshow('Marking ROI',img1)
        k = cv2.waitKey(33)
        if k == 13:  # Check if Enter key is pressed
            break
    x = cx
    y = cy
    ...
    if max_cl_len > 80:
        ...
        cv2.imwrite(output_test + '/output_' + str(img_name.split('/')[-1]), img3)
    else:
        cv2.imwrite(output_test + '/output_' + str(img_name.split('/')[-1]), img3)
...
def open_file():
    ...
    filetype = imghdr.what(img_name)
    print(filetype)
    if filetype == 'jpeg' or filetype == 'jpg' or filetype == 'png' or filetype == 'tiff' or filetype == 'bmp':
        print('Computing results on: ' + img_name)
        detectLines(img_name)
        status.configure(text = 'Completed')
    else:
        tkinter.messagebox.showinfo('Oh no!', 'Invalid File Type')

window = Tk()
window.title('RNFLD Detection')
window.geometry('600x740')
window.wm_iconbitmap('eyeai.ico')
window.wm_title('RNFLD Detection GUI')
credentials = Canvas(window, width=80, height=80, bg='yellow')
credentials.place(x=520, y=0, width=80, height=80)
logo = Image.open('logo.jpg')
logo = ImageTk.PhotoImage(logo)
credentials.create_image(0, 0, anchor=NW, image=logo)
status = Label(window, text='Please wait...', font=('Arial Italic', 10))
status.place(x=300, y=30, width=150, height=25)
btn = Button(window, text='Browse files', command=lambda: open_file(), bg='pink', fg='blue')
btn.place(x=70, y=30, width=110, height=25)
canvas = Canvas(window, bg='pink', width=600, height=600)
canvas.place(x=0, y=80, width=600, height=600)
resetbtn = Button(window, text='Reset', command=lambda: reset(), bg='pink', fg='blue')
resetbtn.place(x=260, y=695, width=80, height=25)
window.mainloop()
